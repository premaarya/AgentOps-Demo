    ...
    <header>
      ...
      <div className="nav-bar">
        ...
        <div className="mode-toggle">
          <span className={`mode-label${demoMode === "simulated" ? " active" : ""}`}>Simulated</span>
          <button
            className={`toggle-switch${demoMode === "real" ? " active" : ""}`}
            onClick={() => setDemoMode(demoMode === "simulated" ? "real" : "simulated")}
            aria-label="Toggle between simulated and real mode"
          >
            <span className="toggle-knob" />
          </button>
          <span className={`mode-label${demoMode === "real" ? " active" : ""}`}>Real</span>
        </div>
      </div>
