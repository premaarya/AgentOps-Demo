...
const DEFAULT_AGENTS: FoundryAgentInfo[] = [
  { agent_name: "Intake Agent", foundry_agent_id: "-", model: "GPT-4o", status: "registered", tools_count: 3 },
  { agent_name: "Extraction Agent", foundry_agent_id: "-", model: "GPT-4o", status: "registered", tools_count: 3 },
  { agent_name: "Compliance Agent", foundry_agent_id: "-", model: "GPT-4o", status: "registered", tools_count: 3 },
  { agent_name: "Approval Agent", foundry_agent_id: "-", model: "GPT-4o", status: "registered", tools_count: 3 },
];
...
export function DeployDashboard() {
  ...
  async function runDeploy() {
    ...
    if (data) {
      ...
      setResult({
        ...
        summary: { agents_deployed: 4, tools_registered: 12, errors: 0, total_duration_ms: 3200 },
      });
    }
  }
  ...
  return (
    <div className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: "var(--space-lg)" }}>
      ...
      <div>
        <h3 className="section-header" style={{ fontSize: "14px", fontWeight: 600, color: "var(--color-text-secondary)", marginBottom: "var(--space-md)" }}>
          Deployment Pipeline
        </h3>
          ...
          {STAGE_NAMES.map((name, i) => {
            ...
            return (
              <React.Fragment key={name}>
                ...
                {i < STAGE_NAMES.length - 1 && <span className="deploy-arrow">{"\u2192"}</span>}
              </React.Fragment>
            );
          })}
        </div>
            ...
            {result ? (
              agents.map((agent) => (
                <tr key={agent.agent_name}>
                  ...
                  <td>
                    <StatusBadge status={agent.status === "registered" ? "pass" : "fail"} />
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={5} style={{ color: "var(--color-text-disabled)", textAlign: "center", padding: "24px" }}>
      ...
      <div className="security-grid">
        ...
        <div className="security-card">
          ...
          {security.content_safety.map((c) => (
            <div key={c.check} className="security-item">
              <StatusBadge
                status={c.status === "passed" ? "pass" : c.status === "unknown" ? "warn" : "fail"}
              />
              <span>{c.check}</span>
            </div>
          ))}
