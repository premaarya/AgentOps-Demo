...
export function EvaluationLab() {
	...
	async function loadResults() {
		...
		)) as unknown as BaselineComparison | null;
		if (bl) setBaseline(bl);
	}

	async function runSuite() {
		const data = (await post("/evaluations/run", {
			version,
		})) as EvalResult | null;
		if (data && "id" in data) {
			setLatest(data);
		...
		>
			...
			<div className="eval-layout">
				...
				<div className="eval-config">
					...
					<div className="eval-config-value">
						{latest?.total_cases ?? 20} contracts
					</div>
					...
					<div className="eval-config-value" style={{ fontSize: "12px" }}>
						{latest ? new Date(latest.run_at).toLocaleTimeString() : "Never"}
					</div>
					<div className="eval-config-label">Baseline</div>
					<div className="eval-config-value">
						{baseline?.baseline?.version ?? "v1.2"}
					</div>
					...
					{latest && (
						<div
							className={`quality-gate ${latest.quality_gate === "PASS" ? "pass" : "fail"}`}
				...
				>
						...
						<>
							...
							<div className="card">
								...
								>
									<MetricCard
										...
										subtitle={
											Number(metrics.extraction_accuracy) >= 85
												? "PASS"
												: "FAIL"
										}
										trend={
											Number(metrics.extraction_accuracy) >= 85 ? "up" : "down"
										}
									/>
									<MetricCard
										label="Compliance"
										...
										subtitle={
											Number(metrics.compliance_accuracy) >= 80
												? "PASS"
												: "WARN"
										}
										trend={
											Number(metrics.compliance_accuracy) >= 80 ? "up" : "down"
										}
									/>
									<MetricCard
										...
										subtitle={
											Number(metrics.classification_accuracy) >= 90
												? "PASS"
												: "FAIL"
										}
										trend={
									...
									<MetricCard
										...
										subtitle={
											Number(metrics.false_flag_rate) <= 15 ? "PASS" : "WARN"
										}
										trend={
											Number(metrics.false_flag_rate) <= 15 ? "up" : "down"
										}
									/>
									<MetricCard
										label="Latency P95"
										...
										subtitle={
											Number(metrics.latency_p95_s) <= 5 ? "PASS" : "WARN"
										}
							...
							{judge && (
								<div className="card">
									...
									>
										{(["relevance", "groundedness", "coherence"] as const).map(
											(dim) => (
					...
					{baseline?.delta && latest && (
						<div className="card">
							<div className="card-header">
								Baseline Comparison ({baseline.baseline.version} vs{" "}
								{latest.version})
							</div>
								...
								<tbody>
									<tr>
										...
										<td
											style={{
												color:
													baseline.delta.accuracy >= 0
														? "var(--color-pass)"
														: "var(--color-fail)",
											}}
										>
											{baseline.delta.accuracy >= 0 ? "+" : ""}
											{baseline.delta.accuracy}%
										</td>
									</tr>
										...
										<>
											<tr>
												...
												<td
													style={{
														color:
															baseline.delta.relevance >= 0
																? "var(--color-pass)"
																: "var(--color-fail)",
													}}
												>
													{baseline.delta.relevance >= 0 ? "+" : ""}
													{baseline.delta.relevance}
												</td>
											</tr>
											<tr>
												<td>Groundedness</td>
												...
												<td
													style={{
														color:
															baseline.delta.groundedness >= 0
																? "var(--color-pass)"
																: "var(--color-fail)",
													}}
												>
													{baseline.delta.groundedness >= 0 ? "+" : ""}
													{baseline.delta.groundedness}
												</td>
											</tr>
											<tr>
												<td>Coherence</td>
												...
												<td
													style={{
														color:
															baseline.delta.coherence >= 0
																? "var(--color-pass)"
																: "var(--color-fail)",
													}}
												>
													{baseline.delta.coherence >= 0 ? "+" : ""}
													{baseline.delta.coherence}
												</td>
											</tr>
										</>
									)}
								...
								<tbody>
									{[
										...
										<tr key={row.id}>
											...
											<td>
												<StatusBadge status={row.ext} />
											</td>
											<td>
												<StatusBadge status={row.comp} />
											</td>
											<td>{row.judge}</td>
											<td>
