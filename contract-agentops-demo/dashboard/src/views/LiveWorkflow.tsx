...
import { type WsMessage, useWebSocket } from "../hooks/useWebSocket.js";
...
export function LiveWorkflow() {
	...
	const [inputMode, setInputMode] = useState<"sample" | "paste" | "file">(
		"sample",
	);
	...
	async function loadSampleText(filename: string) {
		try {
			const res = await fetch(
				`/api/v1/sample-contracts/${encodeURIComponent(filename)}`,
			);
	...
	const showHitl =
		(approvalResult?.action === "escalate_to_human" ||
			messages.some((m) => m.event === "hitl_required")) &&
		!hitlResolved;

	// Get real compliance flags from results
	const complianceFlags = (() => {
		if (!complianceResult) return [];
		const results = (complianceResult.clauseResults ??
			complianceResult.clause_results ??
			[]) as Array<{
			clause?: string;
			type?: string;
			status?: string;
				...
				>
					<div style={{ display: "flex", gap: "4px" }}>
						{(["sample", "paste", "file"] as const).map((mode) => (
							<button
								key={mode}
								className={`btn btn-ghost${inputMode === mode ? " active" : ""}`}
								style={{
									fontSize: "11px",
									padding: "4px 8px",
									textTransform: "capitalize",
									background:
										inputMode === mode ? "var(--color-accent)" : undefined,
			...
			<div className="workflow-canvas">
				{STAGES.map((stage, i) => {
					const msg = messages.find(
						(m) => m.agent === stage && m.event === "agent_step_complete",
					);
					const isActive =
						messages.some(
							(m) => m.agent === stage && m.event !== "agent_step_complete",
					...
					return (
						<React.Fragment key={stage}>
							<div
								className={`workflow-node${isCompleted ? " completed" : ""}${isActive ? " active" : ""}`}
							>
								<div
			...
			{intakeResult && (
				<div className="contract-details-bar">
					<span>
						...
						{String(intakeResult.type ?? intakeResult.classification ?? "—")}
					</span>
					<span>
						<strong style={{ color: "var(--color-text-primary)" }}>
							Parties:
						</strong>{" "}
						{Array.isArray(intakeResult.parties)
							? (intakeResult.parties as string[]).join(", ")
							: String(intakeResult.parties ?? "—")}
					</span>
					<span>
						<strong style={{ color: "var(--color-text-primary)" }}>
							Confidence:
						...
						{String(
							intakeResult.confidence ??
								intakeResult.classification_confidence ??
								"—",
						)}
					</span>
					<span>
						...
						{String(intakeResult.risk_level ?? intakeResult.risk ?? "—")}
					</span>
				</div>
			)}
			...
			{extractionResult && (
				...
				>
					...
					<span style={{ color: "var(--color-text-secondary)" }}>
						{Array.isArray(extractionResult.clauses)
							? `${(extractionResult.clauses as unknown[]).length} clauses`
							: ""}
						{extractionResult.parties
							? ` | Parties: ${Array.isArray(extractionResult.parties) ? (extractionResult.parties as string[]).join(", ") : extractionResult.parties}`
							: ""}
						{extractionResult.values
							? ` | Values: ${JSON.stringify(extractionResult.values)}`
							: ""}
					</span>
				</div>
			)}
				...
				{messages.length > 0 ? (
					messages.map((msg, i) => (
						<div key={i} className="activity-log-entry">
							...
							{msg.result != null && (
								<JsonViewer data={msg.result} maxHeight="100px" />
							)}
						</div>
					))
				) : (
				...
				<div className="hitl-panel">
					...
					>
						...
						{String(
							approvalResult?.reason ??
								"Compliance flagged issues requiring review",
						)}
					</div>
						...
						<div className="hitl-flagged">
							{complianceFlags.map((flag, i) => (
								<div key={i} className="hitl-flag">
									...
									<span>
										{flag.clause ?? flag.type ?? "Flag"}:{" "}
										{flag.reason ?? "Review needed"}
									</span>
								</div>
							))}
						</div>
					...
					<div className="hitl-actions">
						<button
							...
							onClick={() => resolveHitl("approved")}
						>
							Approve
						</button>
						<button
							className="btn btn-danger"
							onClick={() => resolveHitl("rejected")}
						>
							Reject
						</button>
						<button
							className="btn btn-warning"
							onClick={() => resolveHitl("changes")}
						>
							Request Changes
						</button>
						<input
			...
			{hitlResolved && (
				<div
					style={{
						...
						padding: "var(--space-md)",
						background:
							hitlResolved === "approved"
								? "rgba(0,178,148,0.1)"
								: hitlResolved === "rejected"
									? "rgba(231,76,60,0.1)"
									: "rgba(255,185,0,0.1)",
