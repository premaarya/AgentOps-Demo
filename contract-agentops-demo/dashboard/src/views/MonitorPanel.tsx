...
export function MonitorPanel() {
	...
	const tracesByAgent = traces.reduce<Record<string, TraceEntry[]>>(
		(acc, t) => {
			(acc[t.agent] ??= []).push(t);
			return acc;
		},
		...
		<div className="animate-fade-in">
			...
			{monitorData && (
				<div className="metric-grid">
					<MetricCard
						label="Total Tokens"
						value={monitorData.total_tokens.toLocaleString()}
					/>
					<MetricCard
						label="Total Cost"
						value={`$${monitorData.total_cost.toFixed(4)}`}
					/>
					<MetricCard
						label="Latency"
						value={`${(monitorData.total_latency_ms / 1000).toFixed(1)}s`}
					/>
				...
				<>
					...
					>
						...
						{Object.keys(tracesByAgent).length === 0 ? (
							...
							>
								No trace data available for this contract.
							</div>
						) : (
					...
					>
						...
						>
							{agents.map((a) => {
								const pct =
									totalLatency > 0 ? (a.latency_ms / totalLatency) * 100 : 0;
								const cls =
									a.latency_ms < 1000
										? "var(--color-pass)"
										: a.latency_ms < 3000
								...
								return (
									<div key={a.agent}>
										...
										>
											...
											>
												{(a.latency_ms / 1000).toFixed(1)}s ({pct.toFixed(0)}%)
											</span>
										</div>
										<div
											style={{
						...
						>
							...
							<tbody>
								...
								>
									...
									>
										{monitorData.total_tokens.toLocaleString()}
									</td>
									<td
										style={{
											textAlign: "right",
									...
									>
										${monitorData.total_cost.toFixed(4)}
									</td>
								</tr>
							</tbody>
						</table>
								...
								{monitorData.audit_trail.map((entry, i) => (
									<div
										...
										style={{
											...
											borderBottom:
												i < monitorData.audit_trail.length - 1
