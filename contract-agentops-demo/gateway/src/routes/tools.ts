...
async function callMcpTool(
  ...
  try {
    ...
    if (!sseRes.ok || !sseRes.body) {
      return { result: null, error: `SSE connection failed (${sseRes.status})` };
    }
    ...
    const rpcRes = await fetch(
      `http://localhost:${port}/messages?sessionId=${encodeURIComponent(sessionId)}`,
      {
        method: "POST",
    ...
    if (!rpcRes.ok) {
      ...
      reader.cancel().catch(() => {});
      return { result: null, error: `RPC failed (${rpcRes.status}): ${errText}` };
    }
