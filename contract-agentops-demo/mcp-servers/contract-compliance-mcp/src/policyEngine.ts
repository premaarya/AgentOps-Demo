...
class DynamicPolicyEngine {
  ...
  async evaluateClause(clauseType: string, clauseText: string, extractedData?: Record<string, any>): Promise<PolicyViolation[]> {
    if (!this.lastLoaded || Date.now() - this.lastLoaded.getTime() > 300000) { // Refresh every 5 minutes
  ...
  private extractJurisdiction(text: string): string {
    const jurisdictions = [
      ...
      "Singapore", "London", "Hong Kong", "Ontario", "Quebec"
    ];
  ...
  private async initializeDefaultPolicies(): Promise<void> {
    this.policies = [
      ...
      },
      {
        ...
        severity: "medium",
        message_template: "Data retention period of {actual_value} years exceeds policy maximum of {policy_value} years",
        effective_date: "2024-01-01",
        enabled: true
      },
      {
