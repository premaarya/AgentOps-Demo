...
import { z } from "zod";
import { addPolicyRule, checkPolicy, deletePolicyRule, flagRisk, getPolicyRules, updatePolicyRule } from "./engine.js";
import type { PolicyRule } from "./policyEngine.js";

const PORT = Number.parseInt(process.env.MCP_COMPLIANCE_PORT ?? "9003", 10);
...
server.tool("get_policy_rules", "Retrieve company policy rules for compliance checking", {}, async () => {
			...
			content: [
				{
					...
					text: JSON.stringify({
						success,
						message: success ? "Policy rule updated successfully" : "Policy rule not found",
					}),
				},
			],
		};
			...
			content: [
				{
					...
					text: JSON.stringify({
						success,
						message: success ? "Policy rule deleted successfully" : "Policy rule not found",
					}),
				},
			],
		};
