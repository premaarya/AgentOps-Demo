...
import { runPipeline, storeTraces, getTraces } from "../gateway/src/orchestrator/pipeline.js";
...
class TestAdapter implements ILlmAdapter {
  async complete(req: LlmRequest): Promise<LlmResponse> {
    ...
    if (lp.includes("classify") || lp.includes("intake")) {
      content = JSON.stringify({ type: "NDA", confidence: 0.95, metadata: { parties: ["A", "B"] } });
    } else if (lp.includes("extract") || lp.includes("clause")) {
      content = JSON.stringify({
        clauses: [
          { type: "confidentiality", text: "Shall not disclose...", section: "3" },
...
describe("pipeline integration", () => {
  ...
  it("stores and retrieves traces", () => {
    storeTraces("test-trace-id", [
      { id: "t1", contract_id: "test-trace-id", agent: "intake", tool: "classify_document", input: {}, output: {}, latency_ms: 10, tokens_in: 1, tokens_out: 1, timestamp: new Date().toISOString() },
    ]);
