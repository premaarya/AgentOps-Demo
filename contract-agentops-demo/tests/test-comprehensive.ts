...
async function testSuite() {
  ...
  await runTest('Agent Registration System', () => {
    ...
    return (
      ...
      expectedAgents.every(name => 
        result.agents.some(agent => agent.agent_name === name)
      )
    );
  });
  ...
  await runTest('Security Configuration Validation', () => {
    ...
    return (
      requiredSecurityChecks.every(check =>
        result.security.identity_access.some(item => 
          item.check === check && item.status === 'passed'
        )
      ) &&
      requiredSafetyChecks.every(check =>
        result.security.content_safety.some(item =>
          item.check === check && item.status === 'passed'
        )
      )
    );
    ...
    try {
      ...
      return (
        ...
        result.stages.every(stage => 
          ...
          ['passed', 'failed', 'skipped'].includes(stage.status)
        )
      );  
    } catch (error) {
  ...
  await runTest('API Response Data Structure', () => {
    ...
    const stagesValid = result.stages.every(stage =>
      typeof stage.name === 'string' &&
      typeof stage.status === 'string' &&
      typeof stage.duration_ms === 'number'
    );
    
    const agentsValid = result.agents.every(agent =>
