...
document.addEventListener('DOMContentLoaded', function () {
  ...
  tabs.forEach(function (tab, index) {
    tab.addEventListener('click', function () { switchView(index); });
  ...
  dots.forEach(function (dot, index) {
    dot.addEventListener('click', function () { switchView(index); });
  });
...
function updateToolList() {
  ...
  var tools = (dashboardMode === 'real' && window.realMcpTools) ? (window.realMcpTools[server] || []) : (mcpTools[server] || []);
...
function resolveHitl(decision) {
  ...
  var statusMap = {
    ...
    changes: { text: 'Changes Requested', badge: 'warn', log: 'Requested changes' }
  };

  var result = statusMap[decision];
  setNodeState('wf-approval', decision === 'approved' ? 'complete' : (decision === 'rejected' ? 'warning' : 'warning'), result.text);
...
function refreshMonitor() {
  if (dashboardMode === 'real') {
    ...
    var contractId = select ? select.value : '';
...
function runEvalSuite() {
  ...
  var metrics = [
    { id: 'm-extraction', value: '91.2%', delta: '+3.1%', baseline: '91.2%', bdelta: '+3.1%', bId: 'b-extraction', dId: 'd-extraction' },
    { id: 'm-compliance', value: '87.5%', delta: '+5.2%', baseline: '87.5%', bdelta: '+5.2%', bId: 'b-compliance', dId: 'd-compliance' },
    { id: 'm-classification', value: '95.0%' },
    { id: 'm-false-flags', value: '8.5%', baseline: '8.5%', bdelta: '-4.1%', bId: 'b-false-flags', dId: 'd-false-flags' },
    { id: 'm-latency', value: '3.1s', baseline: '3.1s', bdelta: '-0.4s', bId: 'b-latency', dId: 'd-latency' }
  ];

  // LLM-as-judge scores
  var judgeMetrics = [
    { id: 'j-relevance', value: '4.6 / 5', baseline: '4.6/5', bdelta: '+0.4', bId: 'b-relevance', dId: 'd-relevance' },
    { id: 'j-groundedness', value: '4.3 / 5', baseline: '4.3/5', bdelta: '+0.3', bId: 'b-groundedness', dId: 'd-groundedness' },
    { id: 'j-coherence', value: '4.8 / 5', baseline: '4.8/5', bdelta: '+0.3', bId: 'b-coherence', dId: 'd-coherence' }
  ];
  ...
  metrics.forEach(function (m, i) {
    setTimeout(function () {
      ...
      if (m.bId) {
        ...
        deltaEl.classList.add(m.bdelta.charAt(0) === '+' ? 'positive' : (m.bdelta.charAt(0) === '-' && m.id !== 'm-false-flags' && m.id !== 'm-latency' ? 'negative' : 'positive'));
      }
    }, (i + 1) * 300);
  });
