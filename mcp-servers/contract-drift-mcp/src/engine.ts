...
export async function detectDataDrift(): Promise<{
  ...
  return {
    ...
    recommendation: newTypes.length > 0
...
export async function simulateModelSwap(): Promise<{
  ...
  candidate: { accuracy: number; latency_ms: number; cost_per_contract: number };
  ...
  const verdict = Math.abs(swap.comparison.accuracy_delta) <= accuracyThreshold
    ? "ACCEPTABLE" as const
    : "DEGRADED" as const;
