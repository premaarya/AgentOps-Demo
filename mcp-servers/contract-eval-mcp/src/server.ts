...
import { z } from "zod";
import { getBaseline, getEvalResults, getGroundTruth, runEvalSuite } from "./engine.js";

const PORT = Number.parseInt(process.env.MCP_EVAL_PORT ?? "9006", 10);
...
server.tool("get_results", "Retrieve all evaluation results from previous runs", {}, async () => {
	...
	async ({ current_version }) => {
		...
		const comparison = {
			baseline: {
				...
				judge_scores: baseline.judge_scores,
			},
			current: {
			...
			delta: {
				...
				relevance: current.judge_scores
					? Math.round((current.judge_scores.relevance - baseline.judge_scores.relevance) * 10) / 10
					: 0,
				groundedness: current.judge_scores
					? Math.round((current.judge_scores.groundedness - baseline.judge_scores.groundedness) * 10) / 10
					: 0,
				coherence: current.judge_scores
					? Math.round((current.judge_scores.coherence - baseline.judge_scores.coherence) * 10) / 10
					: 0,
			},
		};
		return {
